...
class Request {
  ...
  send(connection) {
    // The parameter connection is a constructed TCP connection.
    return new Promise((resolve, reject) => {
      ...
      if (connection) {
        ...
        connection = net.createConnection(
          ...
          () => {
            connection.write(this.toString());
          } // A callback function when the connection is created successfully
        );
      }
      connection.on("data", (data) => {
        // listen to the connection's data field
...
class ChunkedBodyParser {
  ...
  receiveChar(char) {
    if (this.current === this.WAITING_LENGTH) {
      ...
      if (this.length !== 0) {
        // make sure it's volid to push a new char to the content
